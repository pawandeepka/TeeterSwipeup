...
public class CardStackViewRight extends FrameLayout {

    public interface CardEventListener {
        void onCardDraggingRight(float percentX, float percentY);
        void onCardSwipedRight(SwipeDirection direction);
        void onCardReversedRight();
        void onCardMovedToOriginRight();
        void onCardClickedRight(int index);
    }
    ...
    private LinkedList<CardContainerViewRight> containers = new LinkedList<>();
    ...
    private CardContainerViewRight.ContainerEventListener containerEventListener = new CardContainerViewRight.ContainerEventListener() {
        @Override
        ...
        public void onContainerMovedToOrigin() {
            ...
            if (cardEventListener != null) {
                cardEventListener.onCardMovedToOriginRight();
            }
        }
        @Override
        public void onContainerClicked() {
            if (cardEventListener != null) {
                cardEventListener.onCardClickedRight(state.topIndex);
            }
        }
    };

    public CardStackViewRight(Context context) {
        this(context, null);
    }

    public CardStackViewRight(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CardStackViewRight(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void initializeViews() {
        ...
        for (int i = 0; i < option.visibleCount; i++) {
            CardContainerViewRight view = (CardContainerViewRight) LayoutInflater.from(getContext())
                    .inflate(R.layout.card_container_right, this, false);
            view.setDraggable(false);
            view.setCardStackOption(option);
            view.setOverlayRight(option.rightOverlay, option.bottomOverlay, option.topOverlay);
    ...
    private void initializeViewContents() {
        for (int i = 0; i < option.visibleCount; i++) {
            CardContainerViewRight container = containers.get(i);
    ...
    private void loadNextView() {
        ...
        if (hasNextCard) {
            CardContainerViewRight container = getBottomView();
    ...
    private void clear() {
        for (int i = 0; i < option.visibleCount; i++) {
            CardContainerViewRight view = containers.get(i);
            view.reset();
    ...
    private void update(float percentX, float percentY) {
        if (cardEventListener != null) {
            cardEventListener.onCardDraggingRight(percentX, percentY);
        }
    ...
    public void performReverse(Point point, View prevView, final Animator.AnimatorListener listener) {
        ...
        CardContainerViewRight topView = getTopView();
    ...
    public void performSwipe(SwipeDirection direction, AnimatorSet set, final Animator.AnimatorListener listener) {
        ...
        set.setInterpolator(new TimeInterpolator() {
            ...
            public float getInterpolation(float input) {
                CardContainerViewRight view = getTopView();
    ...
    private void moveToBottom(CardContainerViewRight container) {
        CardStackViewRight parent = (CardStackViewRight) container.getParent();
        if (parent != null) {
    ...
    private void moveToTop(CardContainerViewRight container, View child) {
        CardStackViewRight parent = (CardStackViewRight) container.getParent();
        if (parent != null) {
            parent.removeView(container);
    ...
    private void reorderForReverse(View prevView) {
        CardContainerViewRight bottomView = getBottomView();
    ...
    private void executePostSwipeTask(Point point, SwipeDirection direction) {
        ...
        if (cardEventListener != null) {
            cardEventListener.onCardSwipedRight(direction);
        }
    ...
    private void executePostReverseTask() {
        ...
        if (cardEventListener != null) {
            cardEventListener.onCardReversedRight();
        }
    ...
    public CardContainerViewRight getTopView() {
        return containers.getFirst();
    }

    public CardContainerViewRight getBottomView() {
